...
export default function VisionSection() {
  ...
  useGSAP(
    () => {
      if (!sectionRef.current || !titleRef.current || !contentRef.current || !titlePlaceholderRef.current) return;

      let scrollTriggerInstance: ScrollTrigger | null = null;

      // Wait for content to render, then calculate pin distance
      const updatePinDistance = () => {
        if (!contentRef.current || !titleRef.current || !titlePlaceholderRef.current) return;
  ...
  return (
    ...
    >
      <div className="relative z-10 mx-auto flex h-full w-full flex-col">
        <div ref={containerRef} className="flex w-full flex-col gap-4 md:flex-row md:items-start md:gap-8">
          {/* Left section - Title */}
          ...
          <div ref={contentRef} className="flex w-full flex-col md:w-1/2 md:mt-60">
            {/* Top border */}
